...
#define N_DEFPARSER(name,inner) struct name; HParser *hammer_##name(); void print_ ## name(const struct name *val, FILE *out,int indent); int gen_##name(HBitWriter *out,struct name *val); extern  struct  name * parse_## name(const uint8_t* input, size_t length);
