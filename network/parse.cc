...
int internet_checksum_generate(NailArena *tmp, NailOutStream *current, uint16_t *ptr_checksum){
  uint32_t checksum=0;
  ...
  *ptr_checksum = htons(~((uint16_t)checksum));
  return 0;
}
template <typename str> struct ip_header_checksum_parse {
};
template <> struct ip_header_checksum_parse<NailMemStream> {
  static int f(NailArena *tmp,NailMemStream *in_current,uint16_t *ptr_checksum, uint8_t *ihl){
    if(*ihl* 4 > in_current->getSize()) return -1;
    NailMemStream total_header(in_current->getBuf(), *ihl*4);
    return internet_checksum_parse<NailMemStream>::f(tmp,&total_header, ptr_checksum);        
  }
};

int ip_header_checksum_generate(NailArena *tmp, NailOutStream *current, uint16_t *ptr_checksum, uint8_t *ihl){
  if(*ihl* 4 > current->size) return -1;
  NailOutStream total_header = {.data = current->data,
                                .size = (size_t)*ihl*4,
                                .pos = (size_t) *ihl*4,
                                .bit_offset = 0};
  return internet_checksum_generate(tmp,&total_header, ptr_checksum);        
}
...
template <> struct ip_header_parse<NailMemStream> {
  ...
  static int f(NailArena *tmp, NailMemStream **out_options,NailMemStream **out_body,NailMemStream *in_current, uint8_t *ihl, uint16_t *total_length,uint8_t *protocol){
...
int ip_header_generate(NailArena *tmp,  const NailOutStream* in_options, const NailOutStream *in_body, NailOutStream *current, uint8_t *ihl,uint16_t *total_length, uint8_t *protocol)
{
