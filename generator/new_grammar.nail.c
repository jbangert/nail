...
number*parse_number(NailArena *arena, const uint8_t *data, size_t size) {
    ...
    tr_ptr = trace.trace;
    if(parser_fail(peg_number(&trace,&stream))) return NULL;
...
varidentifier*parse_varidentifier(NailArena *arena, const uint8_t *data, size_t size) {
    ...
    n_trace_init(&trace,4096,4096);
    tr_ptr = trace.trace;
    if(parser_fail(peg_varidentifier(&trace,&stream))) return NULL;
    retval =  (typeof(retval))n_malloc(arena,sizeof(*retval));
    if(!retval) return NULL;
    stream.pos = 0;
...
constidentifier*parse_constidentifier(NailArena *arena, const uint8_t *data, size_t size) {
    ...
    n_trace_init(&trace,4096,4096);
    tr_ptr = trace.trace;
    if(parser_fail(peg_constidentifier(&trace,&stream))) return NULL;
    retval =  (typeof(retval))n_malloc(arena,sizeof(*retval));
    if(!retval) return NULL;
    stream.pos = 0;
...
streamidentifier*parse_streamidentifier(NailArena *arena, const uint8_t *data, size_t size) {
    ...
    n_trace_init(&trace,4096,4096);
    tr_ptr = trace.trace;
    if(parser_fail(peg_streamidentifier(&trace,&stream))) return NULL;
    retval =  (typeof(retval))n_malloc(arena,sizeof(*retval));
    if(!retval) return NULL;
    stream.pos = 0;
...
dependencyidentifier*parse_dependencyidentifier(NailArena *arena, const uint8_t *data, size_t size) {
    ...
    n_trace_init(&trace,4096,4096);
    tr_ptr = trace.trace;
    if(parser_fail(peg_dependencyidentifier(&trace,&stream))) return NULL;
    retval =  (typeof(retval))n_malloc(arena,sizeof(*retval));
    if(!retval) return NULL;
    stream.pos = 0;
...
intconstant*parse_intconstant(NailArena *arena, const uint8_t *data, size_t size) {
    ...
    n_trace_init(&trace,4096,4096);
    tr_ptr = trace.trace;
    if(parser_fail(peg_intconstant(&trace,&stream))) return NULL;
    retval =  (typeof(retval))n_malloc(arena,sizeof(*retval));
    if(!retval) return NULL;
    stream.pos = 0;
...
intp*parse_intp(NailArena *arena, const uint8_t *data, size_t size) {
    ...
    n_trace_init(&trace,4096,4096);
    tr_ptr = trace.trace;
    if(parser_fail(peg_intp(&trace,&stream))) return NULL;
    retval =  (typeof(retval))n_malloc(arena,sizeof(*retval));
    if(!retval) return NULL;
    stream.pos = 0;
...
constint*parse_constint(NailArena *arena, const uint8_t *data, size_t size) {
    ...
    n_trace_init(&trace,4096,4096);
    tr_ptr = trace.trace;
    if(parser_fail(peg_constint(&trace,&stream))) return NULL;
    retval =  (typeof(retval))n_malloc(arena,sizeof(*retval));
    if(!retval) return NULL;
    stream.pos = 0;
...
arrayvalue*parse_arrayvalue(NailArena *arena, const uint8_t *data, size_t size) {
    ...
    n_trace_init(&trace,4096,4096);
    tr_ptr = trace.trace;
    if(parser_fail(peg_arrayvalue(&trace,&stream))) return NULL;
    retval =  (typeof(retval))n_malloc(arena,sizeof(*retval));
    if(!retval) return NULL;
    stream.pos = 0;
...
constarray*parse_constarray(NailArena *arena, const uint8_t *data, size_t size) {
    ...
    n_trace_init(&trace,4096,4096);
    tr_ptr = trace.trace;
    if(parser_fail(peg_constarray(&trace,&stream))) return NULL;
    retval =  (typeof(retval))n_malloc(arena,sizeof(*retval));
    if(!retval) return NULL;
    stream.pos = 0;
...
constfields*parse_constfields(NailArena *arena, const uint8_t *data, size_t size) {
    ...
    n_trace_init(&trace,4096,4096);
    tr_ptr = trace.trace;
    if(parser_fail(peg_constfields(&trace,&stream))) return NULL;
    retval =  (typeof(retval))n_malloc(arena,sizeof(*retval));
    if(!retval) return NULL;
    stream.pos = 0;
...
constparser*parse_constparser(NailArena *arena, const uint8_t *data, size_t size) {
    ...
    n_trace_init(&trace,4096,4096);
    tr_ptr = trace.trace;
    if(parser_fail(peg_constparser(&trace,&stream))) return NULL;
    retval =  (typeof(retval))n_malloc(arena,sizeof(*retval));
    if(!retval) return NULL;
    stream.pos = 0;
...
constraintelem*parse_constraintelem(NailArena *arena, const uint8_t *data, size_t size) {
    ...
    n_trace_init(&trace,4096,4096);
    tr_ptr = trace.trace;
    if(parser_fail(peg_constraintelem(&trace,&stream))) return NULL;
    retval =  (typeof(retval))n_malloc(arena,sizeof(*retval));
    if(!retval) return NULL;
    stream.pos = 0;
...
intconstraint*parse_intconstraint(NailArena *arena, const uint8_t *data, size_t size) {
    ...
    n_trace_init(&trace,4096,4096);
    tr_ptr = trace.trace;
    if(parser_fail(peg_intconstraint(&trace,&stream))) return NULL;
    retval =  (typeof(retval))n_malloc(arena,sizeof(*retval));
    if(!retval) return NULL;
    stream.pos = 0;
...
constrainedint*parse_constrainedint(NailArena *arena, const uint8_t *data, size_t size) {
    ...
    n_trace_init(&trace,4096,4096);
    tr_ptr = trace.trace;
    if(parser_fail(peg_constrainedint(&trace,&stream))) return NULL;
    retval =  (typeof(retval))n_malloc(arena,sizeof(*retval));
    if(!retval) return NULL;
    stream.pos = 0;
...
transform*parse_transform(NailArena *arena, const uint8_t *data, size_t size) {
    ...
    n_trace_init(&trace,4096,4096);
    tr_ptr = trace.trace;
    if(parser_fail(peg_transform(&trace,&stream))) return NULL;
    retval =  (typeof(retval))n_malloc(arena,sizeof(*retval));
    if(!retval) return NULL;
    stream.pos = 0;
...
structparser*parse_structparser(NailArena *arena, const uint8_t *data, size_t size) {
    ...
    n_trace_init(&trace,4096,4096);
    tr_ptr = trace.trace;
    if(parser_fail(peg_structparser(&trace,&stream))) return NULL;
    retval =  (typeof(retval))n_malloc(arena,sizeof(*retval));
    if(!retval) return NULL;
    stream.pos = 0;
...
wrapparser*parse_wrapparser(NailArena *arena, const uint8_t *data, size_t size) {
    ...
    n_trace_init(&trace,4096,4096);
    tr_ptr = trace.trace;
    if(parser_fail(peg_wrapparser(&trace,&stream))) return NULL;
    retval =  (typeof(retval))n_malloc(arena,sizeof(*retval));
    if(!retval) return NULL;
    stream.pos = 0;
...
choiceparser*parse_choiceparser(NailArena *arena, const uint8_t *data, size_t size) {
    ...
    n_trace_init(&trace,4096,4096);
    tr_ptr = trace.trace;
    if(parser_fail(peg_choiceparser(&trace,&stream))) return NULL;
    retval =  (typeof(retval))n_malloc(arena,sizeof(*retval));
    if(!retval) return NULL;
    stream.pos = 0;
...
arrayparser*parse_arrayparser(NailArena *arena, const uint8_t *data, size_t size) {
    ...
    n_trace_init(&trace,4096,4096);
    tr_ptr = trace.trace;
    if(parser_fail(peg_arrayparser(&trace,&stream))) return NULL;
    retval =  (typeof(retval))n_malloc(arena,sizeof(*retval));
    if(!retval) return NULL;
    stream.pos = 0;
...
parameter*parse_parameter(NailArena *arena, const uint8_t *data, size_t size) {
    ...
    n_trace_init(&trace,4096,4096);
    tr_ptr = trace.trace;
    if(parser_fail(peg_parameter(&trace,&stream))) return NULL;
    retval =  (typeof(retval))n_malloc(arena,sizeof(*retval));
    if(!retval) return NULL;
    stream.pos = 0;
...
parameterlist*parse_parameterlist(NailArena *arena, const uint8_t *data, size_t size) {
    ...
    n_trace_init(&trace,4096,4096);
    tr_ptr = trace.trace;
    if(parser_fail(peg_parameterlist(&trace,&stream))) return NULL;
    retval =  (typeof(retval))n_malloc(arena,sizeof(*retval));
    if(!retval) return NULL;
    stream.pos = 0;
...
parameterdefinition*parse_parameterdefinition(NailArena *arena, const uint8_t *data, size_t size) {
    ...
    n_trace_init(&trace,4096,4096);
    tr_ptr = trace.trace;
    if(parser_fail(peg_parameterdefinition(&trace,&stream))) return NULL;
    retval =  (typeof(retval))n_malloc(arena,sizeof(*retval));
    if(!retval) return NULL;
    stream.pos = 0;
...
parameterdefinitionlist*parse_parameterdefinitionlist(NailArena *arena, const uint8_t *data, size_t size) {
    ...
    n_trace_init(&trace,4096,4096);
    tr_ptr = trace.trace;
    if(parser_fail(peg_parameterdefinitionlist(&trace,&stream))) return NULL;
    retval =  (typeof(retval))n_malloc(arena,sizeof(*retval));
    if(!retval) return NULL;
    stream.pos = 0;
...
parserinvocation*parse_parserinvocation(NailArena *arena, const uint8_t *data, size_t size) {
    ...
    n_trace_init(&trace,4096,4096);
    tr_ptr = trace.trace;
    if(parser_fail(peg_parserinvocation(&trace,&stream))) return NULL;
    retval =  (typeof(retval))n_malloc(arena,sizeof(*retval));
    if(!retval) return NULL;
    stream.pos = 0;
...
parserinner*parse_parserinner(NailArena *arena, const uint8_t *data, size_t size) {
    ...
    n_trace_init(&trace,4096,4096);
    tr_ptr = trace.trace;
    if(parser_fail(peg_parserinner(&trace,&stream))) return NULL;
    retval =  (typeof(retval))n_malloc(arena,sizeof(*retval));
    if(!retval) return NULL;
    stream.pos = 0;
...
parser*parse_parser(NailArena *arena, const uint8_t *data, size_t size) {
    ...
    n_trace_init(&trace,4096,4096);
    tr_ptr = trace.trace;
    if(parser_fail(peg_parser(&trace,&stream))) return NULL;
    retval =  (typeof(retval))n_malloc(arena,sizeof(*retval));
    if(!retval) return NULL;
    stream.pos = 0;
...
definition*parse_definition(NailArena *arena, const uint8_t *data, size_t size) {
    ...
    n_trace_init(&trace,4096,4096);
    tr_ptr = trace.trace;
    if(parser_fail(peg_definition(&trace,&stream))) return NULL;
    retval =  (typeof(retval))n_malloc(arena,sizeof(*retval));
    if(!retval) return NULL;
    stream.pos = 0;
...
grammar*parse_grammar(NailArena *arena, const uint8_t *data, size_t size) {
    ...
    n_trace_init(&trace,4096,4096);
    tr_ptr = trace.trace;
    if(parser_fail(peg_grammar(&trace,&stream))) return NULL;
    retval =  (typeof(retval))n_malloc(arena,sizeof(*retval));
    if(!retval) return NULL;
    stream.pos = 0;
