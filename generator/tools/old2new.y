...
constint(A) ::= int(len) EQUAL INT(value). {A=new sstr(); *A<<"h_int_range("<<len <<","<<value <<","<<value<<")";}
...
constparser(A)::=  MANY int(len) EQUAL STRING(value).{eql_assert(len->str(),"h_bits(8,false)"); A=new sstr(); *A<<"N_CONSTANT(h_tok("<<value<<"))";}
constparser(a)::= constint(b). {a= b ;}
constparser(A)::= CONSTIDENTIFIER(name). {A=new sstr(); *A<< "N_EMBED("<<name<<")";}
constparser(A)::= MANY constparser(b). {A=new sstr(); *A<<"N_ARRAY("<<b<<",h_many)";}
        
constparser(A)::= constparser(a) OR constint(b). {A=new sstr(); *A<<"h_choice("<<a<<","<<b<<")";}
...
field(a) ::= constparser(b). {a=new sstr(); *a<< b;}
