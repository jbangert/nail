...
\cc{u16\_depend} transform to check whether the two values are equal. Unlike
most other transforms, \cc{u16\_depend} does not consume or produce strings; it
...
XXX refer back to \S\ref{s:motivation}.
...
expose any untrusted pointers to the application, it protects against
memory corruption attacks by design.

Nail also protects against parser ambiguity vulnerabilities.

Instead of both components containing
