...
Figure~\ref{fig:eval-protocols} lists various example protocols we have implemented.

\begin{figure}[tb]
\label{fig:eval-protocols}
\begin{tabular}{@{}lll@{}}
\toprule
\textbf{Protocol} & \textbf{LoC} & \textbf{Problematic features} \\ 
\midrule
...
\end{tabular}
\caption{Example Nail grammars, their code size of the grammar plus  the transformations they use
  and protocol features that cannot be expressed in existing grammar languages. }
\end{figure}
\subsubsection{DNS.}
...
&&  ~10.000 (DJBDNS)\\ 
DNS \texttt{host} tool & 210  & \XXX[TODO]\\
