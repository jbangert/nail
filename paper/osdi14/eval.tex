...
application that uses Nail, we implemented three applications---a
DNS server, a replacement for the \cc{host} DNS client tool, and an
...
SlocCount~\cite{sloccount}.  We also compare the code size of our
...
\XXX[nz][should we kill \cc{host}, since our replacement is nowhere
near as full-featured as the real \cc{host} or \cc{dig}?]
...
Our DNS grammar, which is partly reproduced in Figure~\ref{fig:dns-full},
consists of a 48 line Nail grammar and 64 lines of C
implementing DNS label compression, whose signatures are shown in
Figure~\ref{fig:dns-xform}.  The grammar describes both the structure
of DNS packets (36 lines) and of a simple zone-file format (16 lines)
supporting A, NS, MX and CNAME records.
