...
Typically, input parsers, particularly for binary formats, are often
written by hand. Not only is such code plagued by the bugs mentioned above, but
two separate hand-written implementations of the same protocol might disagree on
the meaning of a particular input, which can lead to security
vulnerabilities~\cite{saurik-masterkey}.
