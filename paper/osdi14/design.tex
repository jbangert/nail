...
\paragraph{Optional.}Nail also includes an \cc{optional} combinator, which attempts to recognize a value, but succeeds
...
\label{s:transforms}
Traditional parsers handle input one symbol at a time, beginning to end.
...
processing before the end of input has been reached and perhaps continue executing a parent parser.
...
Nail generates always operate on an implicit stream named \cc{\$current} that they process front to
...
inserting the result in the data model. Streams can be defined in two ways.

First, a rule can require streams as arguments, which can be passed in by the program that invokes
the generated parser.Every rule needs toe passed at least the implicit current stream.

Second, the programmer can create temporary streams in the grammars with \emph{transformations},
which are two arbitrary functions called during parsing and output generation.
...
The generating function has to be an inverse of the parsing function. It takes the same number of temporary streams
the parsing function produces and and writes the same number of streams and dependent field
values the parsing function consumes.


Typically, the top level of most grammars is a rule that only takes a single stream, which may then
