...
static void bind_inner_struct (const HParseResult *p,const HParsedToken *val, inner_struct *out) {
    {
        __typeof__(out) str = out;
        int i =0;
        const HParsedToken **fields = h_seq_elements(val);
        {
            const HParsedToken *val=fields[i];
            __typeof__(str->a1) *out = &(str->a1);
            *out = ((((val->token_type == (HTokenType)TT_UINT) ? (void) (0) : __assert_fail ("val->token_type == (HTokenType)TT_UINT", "grammar.h", 4, __PRETTY_FUNCTION__)), val)->uint);;
            i++;
        } {
            const HParsedToken *val=fields[i];
            __typeof__(str->a2) *out = &(str->a2);
            if(val->token_type == TT_NONE) *out=((void *)0);
            else {
...
HParsedToken *act_inner_struct(const HParseResult *p, void *user_data) {
    inner_struct * out= ((inner_struct *) h_arena_malloc(p->arena, sizeof(inner_struct)));
    const HParsedToken *val = p->ast;
    bind_inner_struct(p,val,out);
    return h_make(p->arena,(HTokenType)TT_inner_struct,out);
}
...
static void bind_obj_array (const HParseResult *p,const HParsedToken *val, obj_array *out) {
    {
        __typeof__(out) str = out;
        int i =0;
        const HParsedToken **fields = h_seq_elements(val);
        {
            const HParsedToken *val=fields[i];
            __typeof__(str->elements) *out = &(str->elements);
            {
                __typeof__(out) arr =out;
                int i;
                const HParsedToken **seq = h_seq_elements(val);
                out->count = h_seq_len(val);
                if(out->count >0) out->elem = (void *)h_arena_malloc(p->arena,sizeof(out->elem[0])*out->count);
...
HParsedToken *act_obj_array(const HParseResult *p, void *user_data) {
    obj_array * out= ((obj_array *) h_arena_malloc(p->arena, sizeof(obj_array)));
    const HParsedToken *val = p->ast;
    bind_obj_array(p,val,out);
    return h_make(p->arena,(HTokenType)TT_obj_array,out);
}
...
static HParser *hammer_x_inner_struct() {
    static HParser *ret=((void *)0);
    if(!ret) {
        ret= h_name("inner_struct",h_sequence(h_name("a1", h_uint8() ), h_name("a2", h_optional(h_uint8()) ), ((void *)0)));
    }
    return ret;
}
HParser *hammer_inner_struct() {
    static HParser *ret=((void *)0);
    if(!ret) {
        ret= h_action(hammer_x_inner_struct(),act_inner_struct,((void *)0));
    }
    return ret;
}
const inner_struct * parse_inner_struct(const uint8_t* input, size_t length) {
    HParseResult * ret = h_parse_error(hammer_inner_struct (), input,length,stderr);
    if(ret && ret->ast) return (const inner_struct *)(ret->ast->user);
    else return ((void *)0);
}
...
static HParser *hammer_x_obj_array() {
    static HParser *ret=((void *)0);
    if(!ret) {
        ret= h_name("obj_array",h_sequence(h_name("elements", h_many(hammer_x_inner_struct()) ), h_end_p(), ((void *)0)));
    }
    return ret;
}
HParser *hammer_obj_array() {
    static HParser *ret=((void *)0);
    if(!ret) {
        ret= h_action(hammer_x_obj_array(),act_obj_array,((void *)0));
    }
    return ret;
}
const obj_array * parse_obj_array(const uint8_t* input, size_t length) {
    HParseResult * ret = h_parse_error(hammer_obj_array (), input,length,stderr);
    if(ret && ret->ast) return (const obj_array *)(ret->ast->user);
    else return ((void *)0);
}
...
void print_inner_struct(const inner_struct *val, FILE *out,int indent) {
    fprintf(out,"\n%*s" "{\"%s\":" ,2*indent,"","inner_struct");;
    indent++;
    {
        ...
        indent++;
        {
            typeof(str->a1) *val = &str->a1;
            fprintf(out,"\n%*s" "\"%s\":" ,2*indent,"","a1");;
            fprintf(out,"%lu",(unsigned long)(*val));
            fprintf(out,",");
        } {
            typeof(str->a2) *val = &str->a2;
            fprintf(out,"\n%*s" "\"%s\":" ,2*indent,"","a2");;
            {
...
void print_obj_array(const obj_array *val, FILE *out,int indent) {
    fprintf(out,"\n%*s" "{\"%s\":" ,2*indent,"","obj_array");;
    indent++;
    {
        __typeof__(val) str = val;
        fprintf(out,"\n%*s" "{" ,2*indent,"");;
        indent++;
        {
            typeof(str->elements) *val = &str->elements;
            fprintf(out,"\n%*s" "\"%s\":" ,2*indent,"","elements");;
            {
                ...
                for(i=0; i<arr->count; i++) {
                    ...
                    print_inner_struct(val,out,indent);
                    if(i+1<arr->count) fprintf(out,",");
                }
                fprintf(out,"\n%*s" "]" ,2*indent,"");
            };
            fprintf(out,",");
...
static void _write(n_outbuf *out, size_t size, void *data)
{
...
static void write_buf_inner_struct (n_outbuf *out,const inner_struct *val) {
    {
        __typeof__(val) str = val;
        {
            const typeof(str->a1) *val = &str->a1;
            _write(out, sizeof(uint8_t), & val);
        } {
            const typeof(str->a2) *val = &str->a2;
            if(val) {
                _write(out, sizeof(uint8_t), & val);
            };
        }
...
char * inner_struct_write(const inner_struct *val, size_t * len) {
    n_outbuf buf;
    buf.begin = malloc(8192);
    buf.write = 0;
    buf.size = 8192;
    write_buf_inner_struct (&buf,val);
    *len = buf.write;
    return buf.begin;
}
...
static void write_buf_obj_array (n_outbuf *out,const obj_array *val) {
    {
        __typeof__(val) str = val;
        {
            const typeof(str->elements) *val = &str->elements;
            {
                int i;
                __typeof__(val->elem) elem =val ;
                for(i=0; i<val->count; i++) {
                    __typeof__(elem) val = elem + i;
                    write_buf_inner_struct(out,val);;
                }
            };
        }
        h_end_p();
...
char * obj_array_write(const obj_array *val, size_t * len) {
    n_outbuf buf;
    buf.begin = malloc(8192);
    buf.write = 0;
    buf.size = 8192;
    write_buf_obj_array (&buf,val);
    *len = buf.write;
    return buf.begin;
}
...
int main()
{
    ...
    if(result) {
        ...
        ((outputsize == inputsize) ? (void) (0) : __assert_fail ("outputsize == inputsize", "test.c", 21, __PRETTY_FUNCTION__));
        ((memcmp(out,input,outputsize)) ? (void) (0) : __assert_fail ("memcmp(out,input,outputsize)", "test.c", 22, __PRETTY_FUNCTION__));
