...
static void bind_foo (const HParseResult *p,const HParsedToken *val, foo *out) {
    {
        ...
        for(i=0; i<out->count; i++) {
            ...
            __typeof__(arr->elem[i]) *out = &arr->elem[i];
            {
                ...
                choice->N_type = val->token_type;
                {
                    ...
                    case CH_A: {
                        __typeof__(choice->CH_A) * out = &choice->CH_A;
                    ...
                    break;
                    case CH_B: {
                        __typeof__(choice->CH_B) * out = &choice->CH_B;
                        *out = ((((val->token_type == (HTokenType)TT_UINT) ? (void) (0) : __assert_fail ("val->token_type == (HTokenType)TT_UINT", "grammar.h", 4, __PRETTY_FUNCTION__)), val)->uint);
                    }
                    break;
                    default:
                        (("This parser generator has a bug.") ? (void) (0) : __assert_fail ("\"This parser generator has a bug.\"", "grammar.h", 4, __PRETTY_FUNCTION__));
                    }
                }
            }
        }
...
HParsedToken *act_foo(const HParseResult *p, void *user_data) {
    foo * out= ((foo *) h_arena_malloc(p->arena, sizeof(foo)));
    const HParsedToken *val = p->ast;
    bind_foo(p,val,out);
    return h_make(p->arena,(HTokenType)TT_foo,out);
}
...
HParsedToken *N_act_choice_tag(const HParseResult *p, void *user_data) {
    return h_make(p->arena,(HTokenType)user_data,p->ast);
}

static HParser *hammer_x_foo() {
    static HParser *ret=((void *)0);
    if(!ret) {
        ret= h_name("foo",h_many(h_choice(h_action(h_ch('a'),N_act_choice_tag,(void *) CH_A), h_action(h_ch('b'),N_act_choice_tag,(void *) CH_B), ((void *)0))));
    }
    return ret;
}
HParser *hammer_foo() {
    static HParser *ret=((void *)0);
    if(!ret) {
        ret= h_action(hammer_x_foo(),act_foo,((void *)0));
    }
    return ret;
}
const foo * parse_foo(const uint8_t* input, size_t length) {
    HParseResult * ret = h_parse_error(hammer_foo (), input,length,stderr);
...
void print_foo(const foo *val, FILE *out,int indent) {
    fprintf(out,"\n%*s" "{\"%s\":" ,2*indent,"","foo");;
    indent++;
    {
        ...
        for(i=0; i<arr->count; i++) {
            __typeof__(arr->elem[i]) *val = &arr->elem[i];
            {
                ...
                switch(choice->N_type) {
                case CH_A: {
                    __typeof__(choice->CH_A) *val = &choice->CH_A;
                    fprintf(out,"\n%*s" "(%s)->" ,2*indent,"","CH_A");;
                    fprintf(out,"%lu",(unsigned long)(*val));
                }
                break;
                case CH_B: {
                    __typeof__(choice->CH_B) *val = &choice->CH_B;
                    fprintf(out,"\n%*s" "(%s)->" ,2*indent,"","CH_B");;
                    fprintf(out,"%lu",(unsigned long)(*val));
                }
                break;
                default:
                    (("We have a bug") ? (void) (0) : __assert_fail ("\"We have a bug\"", "grammar.h", 4, __PRETTY_FUNCTION__));
                }
            };
