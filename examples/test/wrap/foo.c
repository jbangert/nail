...
struct foo;
HParser *hammer_foo();
void print_foo(const struct foo *val, FILE *out,int indent);
int gen_foo(HBitWriter *out,struct foo *val);
...
static void bind_foo (const HParseResult *p,const HParsedToken *val, foo *out) {
    {
        ...
        i++;
        {
            ...
            HParsedToken **fields = h_seq_elements(val);
            {
                ...
                __typeof__(str->test) *out = &(str->test);
                *out = ((((val->token_type == (HTokenType)TT_UINT) ? (void) (0) : __assert_fail ("val->token_type == (HTokenType)TT_UINT", "grammar.h", 7, __PRETTY_FUNCTION__)), val)->uint);;
                i++;
            }
        }
        i++;
...
HParsedToken *N_act_choice_tag(const HParseResult *p, void *user_data) {
    return h_make(p->arena,(HTokenType)user_data,(void *)p->ast);
}

static HParser *hammer_x_foo() {
    static HParser *ret=((void *)0);
    if(!ret) {
        ret = h_name("foo",h_indirect());
        h_bind_indirect(ret,h_name("foo",h_sequence(h_ch('('), h_sequence(h_name("test", h_uint8() ), ((void *)0)), h_ch(')'), ((void *)0))));
    }
    return ret;
}
HParser *hammer_foo() {
