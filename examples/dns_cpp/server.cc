...
struct hashentry *zone_hashtable(zone *zon){
  struct hashentry * table= (hashentry *)calloc(sizeof(struct hashentry),hashsize(zon)); //COLLISIONS
        FOREACH(domain,*zon){
          unsigned long hash = domain_hash((struct domain *)domain);
...
int natoi(unsigned char *s, int n)
{
    int x = 0;
    while(n--)
    {
...
int dns_respond(NailOutStream *stream,NailArena * arena,struct dnspacket *query, zone *zone, struct hashentry *hashtable)
{
        ...
        for(int i=0;i<query->questions.count;i++){
                ...
                unsigned long hash = domain_hash((domain *) &q->labels.domain);
                ...
                for(;hashtable[entry].def != NULL;entry = (entry+1)%hashsize(zone) ){
                        ...
                        if(hashtable[entry].hash != hash)
                                continue;
                        if(domain_cmp(&j->hostname, &q->labels.domain))
                                continue;
                        ...
                        case A:
                                ...
                                rr->rdata.count = 4;
                                rr->rdata.elem = (uint8_t*)n_malloc(arena,4);
                                if(j->rr.a.count < 4)
                                        continue;
                                for(int i=0;i<4;i++)
